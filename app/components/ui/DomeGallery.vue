...
import { GLOBE_IMAGES } from '~/constants/data';
...
const DEFAULT_IMAGES = GLOBE_IMAGES.map(src => ({ src, alt: 'Globe image' }));
...
let autoSpinRAF: number | null = null;
...
function buildItems(pool: (string | ImageItem)[], seg: number): TileItem[] {
  const startX = -Math.floor(seg / 2) * 2;
...
onMounted(() => {
  ...
  const autoSpinSpeed = 0.1;
  const autoSpin = () => {
    if (!draggingRef && inertiaRAF === null && !openingRef && !isEnlarging.value) {
      rotationRef.y = wrapAngleSigned(rotationRef.y + autoSpinSpeed);
      applyTransform(rotationRef.x, rotationRef.y);
    }
    autoSpinRAF = requestAnimationFrame(autoSpin);
  };
  autoSpinRAF = requestAnimationFrame(autoSpin);
});
...
onUnmounted(() => {
  ...
  if (autoSpinRAF !== null) cancelAnimationFrame(autoSpinRAF);
  document.body.classList.remove('dg-scroll-lock');
});
