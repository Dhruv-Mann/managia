...
# UI/UX Fidelity & Design Systems

## Strict Adherence to Design Intent
When recreating a template or adhering to a specific design system (e.g., brutalist, solid-block, high-contrast), **never apply arbitrary modern UI trends** unless explicitly requested by the user.

**DO NOT ADD:**
- Glassmorphism (unnecessary `backdrop-blur` or translucency)
- Unrequested textures (stardust, noise, gradients)
- Arbitrary border-radius changes (e.g., turning sharp brutalist buttons into rounded pills)
- Background images to sections intended to be solid colors

**Respect Graphic Choices:**
If a design features massively oversized typography that clips off the screen or overlaps edges, **do not scale it down just to "make it fit"**. Reproduce the graphic effect exactly as intended. Professionalism means executing the specific design language with pixel-perfect precision, not improvising with generic trends.
